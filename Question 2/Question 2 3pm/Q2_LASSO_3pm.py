...
# load the data
data = pd.read_csv('weather3pm.csv')
...
features = data.drop(['Date', 'Location', 'Temp3pm'], axis=1)
target = data['Temp3pm']
...
plt.title('Lasso regression coefficients Vs. alpha 3pm')
plt.legend(features.columns)
plt.savefig('Different alpha coefficients 3pm', bbox_inches='tight')
...
best_alpha_error = [["Best alpha", best_alpha], [
                     "Minimal error", best_cv_error]]
...
plt.title(('Feature Importance based on Lasso Regression (alpha={})'
           .format(best_alpha)))
plt.xticks(rotation=90)
plt.savefig('Feature importance 3pm', bbox_inches='tight')
